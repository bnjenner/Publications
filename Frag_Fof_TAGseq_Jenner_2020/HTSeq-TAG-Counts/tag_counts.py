...
def count_reads_in_features(sam_filenames, gff_filename,
    ...
    try:
        for f in gff:
            if f.type == feature_type or f.type in tags: # includes all entries with these tags
                ...
                counts[f.attr[id_attribute]] = 0 # the counts dictionary does not include this, meaning that the final counts will not include the added tag
    ...
    if len(counts) == 0:
        sys.stderr.write(
            "Warning: No features of specified types found.\n")
        ...
        try:
            ...
            for r in read_seq:
                ...
                try:
                    ...
                    if fs is not None and len(fs) > 0:  # since fs contains the gene names from the features dictionary, we can  differentiate between UTRs and CDS and prioritize whichever we need 
                       
                        if multimapped_mode == 'none':
                            
                            if len(fs) == 1:  
                                counts[list(fs)[0].split('^')[0]] += 1  # if multimapped counts go to same ID. 
                            
                            elif len(fs) == 2: # if mapping overlaps with two neighboring sequences and one of them is a UTR, count goes to UTR. 
                                    
                                if list(fs)[0].split('^')[-1] == 'UTR' and list(fs)[1].split('^')[-1] != 'UTR':
                                    counts[list(fs)[0].split('^')[0]] += 1 
                            
                                elif list(fs)[1].split('^')[-1] == 'UTR' and list(fs)[0].split('^')[-1] != 'UTR':
...
def main():
    ...
    pa = argparse.ArgumentParser(
        ...
        epilog="Originally written by Simon Anders (sanders@fs.tum.de), " +
        "European Molecular Biology Laboratory (EMBL) and Fabio Zanini " +
        "(fabio.zanini@stanford.edu), Stanford University. (c) 2010-2019. " +
        "Current program includes modifications made by Bradley Jenner " +
        "(bnjenner@ucdavis.edu), University of California, Davis. " +
    ...
    pa.add_argument(
            ...
            help="Specified UTR tag in GTF file (can be comma separated list of tags).")
